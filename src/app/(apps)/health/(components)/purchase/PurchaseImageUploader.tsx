...
export default function PurchaseImageUploader({purchaseId, onUploaded}: Props) {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of Array.from(files)) {
      try {
        ...
        await fetch(`${basePath}/health/api/purchase/${purchaseId}/image`, {
          method: 'POST',
          body: formData,
        })
      } catch (error) {
  ...
  return (
    <div className="border border-dashed border-gray-300 rounded p-2 text-center">
      <input
        ...
        className="hidden"
        id={`purchase-file-${purchaseId}`}
      />
      <label
        htmlFor={`purchase-file-${purchaseId}`}
