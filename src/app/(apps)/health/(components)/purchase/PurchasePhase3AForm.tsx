...
import {PurchaseWithRelations, PurchasePhase3AData} from '../../(lib)/purchase-actions'
...
type Props = {
  ...
  onSubmit: (data: PurchasePhase3AData) => Promise<void>
  onCancel: () => void
  isSubmitting?: boolean
}

export default function PurchasePhase3AForm({purchase, onSubmit, onCancel, isSubmitting = false}: Props) {
  const {session} = useGlobal()
  const [persons, setPersons] = useState<PersonWithRelations[]>([])
  const [formData, setFormData] = useState<PurchasePhase3AData>({
    receiver: '',
    receiverId: (purchase as any)?.receiverId || undefined,
    receivedDate: purchase?.receivedDate
      ? formatDate(new Date(purchase.receivedDate), 'YYYY-MM-DD')
