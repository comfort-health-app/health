...
export default function SymptomImageUploader({symptomId, onUploaded}: Props) {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of Array.from(files)) {
      try {
        ...
        formData.append('symptomId', symptomId.toString())

        await fetch(`${basePath}/health/api/symptom/${symptomId}/image`, {
          method: 'POST',
          body: formData,
        })
      } catch (error) {
  ...
  return (
    <div className="border border-dashed border-gray-300 rounded p-2 text-center">
      <input
        ...
        className="hidden"
        id={`symptom-file-${symptomId}`}
      />
      <label
        htmlFor={`symptom-file-${symptomId}`}
